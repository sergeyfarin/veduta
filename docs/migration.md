...
Keep the binary, embedded frontend, first-party plugin manifests/modules, and approved lock entries on the same release. After upgrading, check `/healthz`, the configuration status, integration status, connection tests, representative cards, rules, and notification delivery before retiring the backup.
