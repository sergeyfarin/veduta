...
[`plugins/immich/manifest.yaml`](../plugins/immich/manifest.yaml) demonstrates asset routes and image output. [`plugins/glances/manifest.yaml`](../plugins/glances/manifest.yaml) demonstrates metrics and signals. [`plugins/beszel/manifest.yaml`](../plugins/beszel/manifest.yaml) demonstrates an authorised record selected from a collection. [`plugins/proxmox/manifest.yaml`](../plugins/proxmox/manifest.yaml) demonstrates aggregating one heterogeneous collection into several signals, and [`plugins/homeassistant/manifest.yaml`](../plugins/homeassistant/manifest.yaml) demonstrates conditional signals - a value that is absent, rather than stale, when there was nothing to read.
