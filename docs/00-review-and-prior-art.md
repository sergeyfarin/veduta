...
| R11 | The example registered Immich as `builtin` while the repo presents it as the flagship declarative integration | **Confirmed, factual error** | Example uses `source: path:../plugins/immich` (resolved from `examples/`), and the manifest is the worked reference |
