...
acceptance, not E1 implementation** · **spec pass DONE; live pass run 2026-09-09 — Immich
confirmed + E3 cold-latency AC met; Jellyfin findings force a manifest/G4 rewrite (open in
`docs/03-backlog.md`)**
...
Blocked as of 2026-09-09: the live S2 pass ran — Immich is confirmed and E3's cold-latency AC is
met — but it invalidated the shipped Jellyfin manifest (`/Users/Me` is unusable with an API key;
`/Items/Latest` is the wrong endpoint). L4 now waits on the coordinated Jellyfin
manifest/spike/capture-script/G4 rewrite tracked in `docs/03-backlog.md`, not on credentials.
