...
acceptance, not E1 implementation** · **DONE — spec pass + live pass (2026-09-09). Immich
...
**E3 · Immich integration — VERTICAL SLICE #1** · 1 d · deps: D3, E1, S2, F1 · **DONE, including
the real-server cold-latency AC** (measured 2026-09-09 against a live Immich 3.1.0 on the LAN,
fresh process + empty disk cache: `GET /api/v1/cards` 31 ms + six thumbnails through the signed
proxy in parallel 58 ms ≈ **90 ms cold**, budget 2 s; the Immich API key was absent from the page
HTML, the card JSON and every log line) (transitively, via E1 -
...
but a Jellyfin API key has no associated user, so that 400s. Recently-added is now a single
`GET /Items?recursive=true&includeItemTypes=Movie,Series&sortBy=DateCreated&sortOrder=Descending`
with no user step (Approach A - see the spike). The module was rebuilt (`sha256 0bd308fd…`), the
lock re-approved, `httpRequests` tightened 5 → 4.
Tests: fixture test against the S2 (now live-shaped) Jellyfin responses produces the shared
...
is reimplemented as builtin Go**, proving the runtime swap is faithful.
...
**L3 · Hardening pass** · 1 d · deps: K2 · **DONE** — CSP and security headers, `go test -fuzz` on
the document validator, manifest parser and token verifier; the import-boundary test
(`integrations/*` may not import `storage`/`secrets`/`connections` internals); a CI secret-scan
asserting no configured secret value appears in any HTTP response; dependency and vulnerability
check (`govulncheck`); load test of 50 cards on a Raspberry Pi 4 class target.
Landed with a header middleware covering success, authentication/error and 404 responses; the SPA
and image handlers narrow its baseline CSP. All three byte/token entry points have seeded native Go
fuzz targets run on every CI push. An AST boundary test rejects persistence/secret imports and any
new connection-layer exception from production integration code. A route-spanning response scan
checks configured values against headers and bodies. CI pins `govulncheck` and runs the real
SQLite-backed 50-card/eight-connection-revision workload on GitHub's native four-core ARM64 runner
under a five-second Pi-class budget.
The post-J atomicity precondition is complete: rule state, transition event, notification outbox
rows, and flood-suspension meta-events now commit together, with crash-boundary rollback tests.
**L4 · Release 0.1.0** · 0.5 d · deps: L1, L2, L3, live S2 validation — multi-arch images, checksums, changelog, generated `THIRD-PARTY-NOTICES.md` covering the frontend and locked Rust plugin dependencies and served from the UI, a "Source" link in the footer satisfying AGPL §13, a short trademark policy, a screenshot generated by
