...
`
	...
	defs, err := Definitions(context.Background(), snap, 1, configPath, immichRegistry{}, map[string]string{"immich": "opaque-revision"}, tokens, slog.Default())
	if err != nil {
		t.Fatal(err)
	}
	if len(defs) != 1 || defs[0].Run == nil {
		t.Fatalf("definitions=%+v", defs)
	}
	doc, err := defs[0].Run(context.Background())
	if err != nil {
		t.Fatal(err)
	}
